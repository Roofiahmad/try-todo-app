...
function Home() {
  ...
  const createActivity = async () => {
    await Activity.create({ title: "New Activity", email: "roofiahmadsidiq@gmail.com" });
    getActivity();
  };
  ...
  return (
    <>
      ...
      {activity.length ? (
        <div className="grid gap-3 pb-10 grid-cols-4">
          {activity.map((ac, index) => (
            <AcCard key={ac.id} index={index} onDelete={(e) => openDeleteModal(e, ac)} {...ac} />
          ))}
        </div>
      ) : (
        <Suspense fallback={<div></div>}>
      ...
      {deleteActivityData && (
        <ModalDelete data={deleteActivityData} onClose={() => setDeleteActivityData(null)} handleDelete={handleDeleteActivity} />
      )}
